...
public abstract class AbstractServerGUI extends JFrame implements IWebServer {
			...
			while (!stop) {
				...
				if (server != null && !server.isStoped()) {
					...
					if (rate == Double.MIN_VALUE)
						AbstractServerGUI.this.txtServiceRate.setText("Unknown");
					else
						AbstractServerGUI.this.txtServiceRate.setText(Double
								.toString(rate));
				}
	...
	private void initComponents() {
		...
		this.txtRootDirectory = new JTextField(rootDirectoryPath);
	...
	protected abstract void setDefaultPortNumber();

	protected int getPortNumber() {
		try {
			this.port = Integer
					.parseInt(AbstractServerGUI.this.txtPortNumber.getText());
		} catch (Exception ex) {
			JOptionPane.showMessageDialog(AbstractServerGUI.this,
	...
	private void addListeners() {
		...
		this.butSelect.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String currentDirectory = AbstractServerGUI.this.txtRootDirectory
						.getText();
				...
				if (fileChooser.showOpenDialog(AbstractServerGUI.this) == JFileChooser.APPROVE_OPTION) {
					// A folder has been chosen
					currentDirectory = fileChooser.getSelectedFile()
							.getAbsolutePath();
					AbstractServerGUI.this.txtRootDirectory.setText(currentDirectory);
				}
			}
		});
		...
		this.butStartServer.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				// Disable widgets
				AbstractServerGUI.this.disableWidgets();
		...
		this.butStopServer.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				AbstractServerGUI.this.enableWidgets();
			}
		});
	...
	public void showSocketException(Exception e) {
		...
		if (AbstractServerGUI.server != null)
			AbstractServerGUI.server.stop();
		AbstractServerGUI.server = null;
